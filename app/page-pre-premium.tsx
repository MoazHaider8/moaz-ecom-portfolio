...
export default function HomePage() {
  ...
  const services = [
    ...
    },
    {
      ...
      description: 'Strategic keyword research mapped to your specific product catalog, category structure, and buyer journey stages. I identify high-intent commercial keywords, informational opportunities, and long-tail variations that your competitors are missing.',
    ...
    },
    {
      ...
      description: 'Optimize your product pages to rank for transactional keywords and convert organic visitors into customers. I implement product schema markup for rich snippets, write conversion-focused meta titles and descriptions, and structure your pages to maximize both rankings and add-to-cart rates.',
    ...
    },
    {
      ...
      description: 'Ecommerce stores face unique technical SEO challenges — faceted navigation creating duplicate content, JavaScript rendering issues, massive product catalogs straining crawl budget, and complex URL structures confusing search engines. I diagnose and fix these platform-specific issues.',
    ...
    },
    {
      ...
      description: 'Category and collection pages are where the highest-volume, most commercially valuable keywords live. I optimize your category page structure, add keyword-targeted content, implement proper internal linking hierarchies, and ensure your taxonomy aligns with how your customers actually search.',
  ...
  return (
    <>
      ...
      <main itemScope itemType="https://schema.org/ProfessionalService">
        ...
        {/* ══════════════════════════════════════════════════
            SECTION 4 — SERVICES (Tabbed Showcase)
        ══════════════════════════════════════════════════ */}
        <section className="py-16 lg:py-20" style={{ background: 'var(--bg)' }}>
            ...
            <div className="grid lg:grid-cols-2 gap-8 items-end mb-14">
              ...
              <div className="flex flex-col lg:items-end gap-4">
                <p className="text-lg max-w-md leading-relaxed lg:text-right" style={{ color: 'var(--text-secondary)' }}>
                  Every service is tailored to increase organic traffic, improve rankings, and grow revenue for your online store.
                </p>
              </div>
            </div>
        ...
        {/* ══════════════════════════════════════════════════
            SECTION 5 — PLATFORMS (Hero Strip + Card Grid)
        ══════════════════════════════════════════════════ */}
        <section className="py-16 lg:py-20 overflow-hidden" style={{ background: 'var(--surface-alt)' }}>
          {/* ── Dark accent hero strip ── */}
          <div className="relative mb-16">
            <div className="absolute inset-0" style={{ background: 'linear-gradient(135deg, var(--card) 0%, var(--surface) 100%)' }} />
            <div className="absolute inset-0 pointer-events-none" style={{ backgroundImage: 'radial-gradient(circle at 1px 1px, var(--accent) 1px, transparent 0)', backgroundSize: '40px 40px', opacity: 0.04 }} />
            <div className="container max-w-7xl mx-auto px-6 relative z-10 py-14 lg:py-20">
              <div className="grid lg:grid-cols-2 gap-8 items-center">
                <div>
                  <span className="text-xs font-semibold uppercase tracking-[0.2em] mb-4 block" style={{ color: 'var(--accent)' }}>
                    Platforms
                  </span>
                  <h2 className="text-4xl lg:text-5xl xl:text-6xl font-bold mb-4" style={{ color: 'var(--text)', lineHeight: '1.1', fontFamily: 'var(--font-heading)' }}>
                    Platform-Specific<br className="hidden lg:block" /> SEO Expertise
                  </h2>
                  <p className="text-base lg:text-lg max-w-xl leading-relaxed" style={{ color: 'var(--text-secondary)' }}>
                    Deep technical knowledge of each platform&apos;s unique SEO architecture, challenges, and optimization opportunities.
                  </p>
                </div>
                {/* Scrolling platform pills */}
                <div className="flex flex-wrap gap-2.5 lg:justify-end">
                  {platforms.map((platform, index) => (
                    <span key={index} className="inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium transition-all duration-300 hover:scale-105" style={{ background: 'var(--surface-alt)', border: '1px solid var(--border)', color: 'var(--text)' }}>
            ...
            <div className="absolute bottom-0 left-0 right-0 h-px" style={{ background: 'linear-gradient(90deg, transparent, var(--accent), transparent)', opacity: 0.3 }} />
          </div>
          ...
          <div className="container max-w-7xl mx-auto px-6">
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {platforms.map((platform, index) => (
                <Link key={index} href={platform.href} className="group relative overflow-hidden rounded-2xl transition-all duration-500 hover:-translate-y-1.5 hover:shadow-2xl" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
                  {/* Animated top accent border */}
                  <div className="h-[3px] w-full transition-all duration-500 group-hover:h-[4px]" style={{ background: `linear-gradient(90deg, ${platform.color}, ${platform.color}80)` }} />

                  {/* Brand-colored glow on hover */}
                  <div className="absolute top-0 left-1/2 -translate-x-1/2 w-48 h-48 pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity duration-700" style={{ background: `radial-gradient(circle, ${platform.color}15, transparent 70%)`, transform: 'translate(-50%, -60%)' }} />

                  <div className="p-7 lg:p-8 relative z-10">
                    {/* Platform icon + label */}
                    <div className="flex items-center gap-3 mb-5">
                      <div className="w-11 h-11 rounded-xl flex items-center justify-center text-base font-bold flex-shrink-0 transition-all duration-300 group-hover:scale-110 group-hover:rotate-3" style={{ backgroundColor: `${platform.color}15`, color: platform.color, border: `1.5px solid ${platform.color}30` }}>
                        {platform.name.charAt(0)}
                      </div>
                      <div>
                        <h3 className="text-lg font-bold leading-tight" style={{ color: 'var(--text)', fontFamily: 'var(--font-heading)' }}>{platform.name}</h3>
                        {index === 0 && (
                          <span className="text-[0.6rem] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full" style={{ background: `${platform.color}20`, color: platform.color }}>Most Popular</span>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-1.5 mb-6">
                      {platform.fixes.map((fix, i) => (
                        <span key={i} className="text-[0.7rem] px-2.5 py-1 rounded-md font-medium" style={{ background: 'var(--surface-alt)', color: 'var(--text)', border: '1px solid var(--border)' }}>
                          {fix}
                        </span>
                      ))}
                    </div>
                    ...
                    <div className="flex items-center justify-between pt-4" style={{ borderTop: '1px solid var(--border)' }}>
                      <span className="text-sm font-semibold transition-all duration-300 group-hover:gap-3 inline-flex items-center gap-2" style={{ color: platform.color }}>
                        Explore
                      ...
                      <span className="text-xs font-mono" style={{ color: 'var(--muted)' }}>{String(index + 1).padStart(2, '0')}/06</span>
                    </div>
                  </div>
                </Link>
              ))}
        ...
        {/* ══════════════════════════════════════════════════
            SECTION 6 — INDUSTRIES (Magazine Bento)
        ══════════════════════════════════════════════════ */}
        <section className="py-16 lg:py-20" style={{ background: 'var(--bg)' }}>
          <div className="container max-w-7xl mx-auto px-6">
            {/* Header with accent rule */}
            <div className="mb-16">
              <div className="flex items-center gap-4 mb-6">
                <div className="h-px flex-1" style={{ background: 'linear-gradient(90deg, var(--accent), transparent)', maxWidth: '80px' }} />
              ...
              <div className="grid lg:grid-cols-2 gap-6 items-end">
                <h2 className="text-4xl lg:text-5xl xl:text-6xl font-bold" style={{ color: 'var(--text)', lineHeight: '1.1', fontFamily: 'var(--font-heading)' }}>
                  Ecommerce SEO Across<br className="hidden lg:block" /> Diverse Industries
                </h2>
                <p className="text-lg leading-relaxed lg:text-right lg:max-w-md lg:ml-auto" style={{ color: 'var(--text-secondary)' }}>
                  Specialized ecommerce SEO knowledge for your specific vertical — because fashion SEO and electronics SEO require very different strategies.
                </p>
              </div>
            </div>
            ...
            <div className="space-y-5">
              ...
              <div className="grid lg:grid-cols-12 gap-5">
                {/* Large featured card */}
                <Link href={industries[0].href} className="lg:col-span-8 group relative overflow-hidden rounded-2xl transition-all duration-500 hover:-translate-y-1 hover:shadow-2xl" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
                  {/* Dot pattern overlay */}
                  <div className="absolute inset-0 pointer-events-none" style={{ backgroundImage: 'radial-gradient(circle at 1px 1px, var(--border) 0.5px, transparent 0)', backgroundSize: '24px 24px', opacity: 0.3 }} />
                  {/* Accent gradient stripe */}
                  <div className="absolute top-0 left-0 right-0 h-1" style={{ background: 'linear-gradient(90deg, var(--accent), var(--accent-hover), transparent)' }} />

                  <div className="relative z-10 p-8 lg:p-12">
                    <div className="grid lg:grid-cols-2 gap-8 items-center">
                      <div>
                        <div className="flex items-center gap-3 mb-5">
                          <span className="text-[0.65rem] font-bold uppercase tracking-wider px-2.5 py-1 rounded-full" style={{ background: 'var(--accent)', color: '#ffffff' }}>
                            Featured
                          </span>
                          <span className="text-xs font-mono" style={{ color: 'var(--muted)' }}>01</span>
                        </div>
                        <h3 className="text-2xl lg:text-3xl font-bold mb-3" style={{ color: 'var(--text)', fontFamily: 'var(--font-heading)' }}>{industries[0].name}</h3>
                        <p className="text-sm leading-relaxed mb-6" style={{ color: 'var(--text-secondary)', lineHeight: '1.8' }}>{industries[0].description}</p>
                      ...
                      {/* Large abbreviation as visual element */}
                      <div className="hidden lg:flex items-center justify-center">
                        <span className="text-[10rem] font-black leading-none select-none" style={{ color: 'var(--accent)', opacity: 0.08, fontFamily: 'var(--font-heading)' }}>
                          {industries[0].abbrev}
                        </span>
                      </div>
                    </div>
                ...
                {/* Small stacked card */}
                <Link href={industries[1].href} className="lg:col-span-4 group relative overflow-hidden rounded-2xl transition-all duration-500 hover:-translate-y-1 hover:shadow-2xl flex flex-col justify-between" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
                  <div className="absolute top-0 left-0 right-0 h-1" style={{ background: 'var(--accent)', opacity: 0.5 }} />
                  {/* Big abbreviation as background */}
                  <div className="absolute -right-3 -top-2 text-[7rem] font-black leading-none select-none pointer-events-none" style={{ color: 'var(--text)', opacity: 0.03, fontFamily: 'var(--font-heading)' }}>
                    {industries[1].abbrev}
                  </div>
                  <div className="relative z-10 p-7 lg:p-8 flex flex-col h-full">
                    <span className="text-xs font-mono mb-4 block" style={{ color: 'var(--muted)' }}>02</span>
                    <h3 className="text-xl font-bold mb-3" style={{ color: 'var(--text)', fontFamily: 'var(--font-heading)' }}>{industries[1].name}</h3>
                    <p className="text-sm leading-relaxed mb-5 flex-1" style={{ color: 'var(--text-secondary)' }}>{industries[1].description}</p>
                    <div className="flex items-center justify-between mt-auto">
                      <span className="text-xs px-3 py-1.5 rounded-full font-medium" style={{ background: 'var(--accent-light)', color: 'var(--accent)', border: '1px solid var(--border)' }}>
                        {industries[1].challenge}
                      </span>
              ...
              <div className="grid md:grid-cols-3 gap-5">
                {industries.slice(2, 5).map((industry, index) => (
                  <Link key={index} href={industry.href} className="group relative overflow-hidden rounded-2xl transition-all duration-500 hover:-translate-y-1 hover:shadow-xl" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
                    {/* Colored top line */}
                    <div className="h-px w-full" style={{ background: 'linear-gradient(90deg, var(--accent), transparent)' }} />
                    {/* Abbreviation watermark */}
                    <div className="absolute -right-2 -bottom-4 text-[6rem] font-black leading-none select-none pointer-events-none" style={{ color: 'var(--text)', opacity: 0.03, fontFamily: 'var(--font-heading)' }}>
                      {industry.abbrev}
                    </div>
                    <div className="relative z-10 p-7">
                      <div className="flex items-center justify-between mb-4">
                        <div className="w-10 h-10 rounded-lg flex items-center justify-center text-xs font-bold" style={{ background: 'var(--accent-light)', color: 'var(--accent)' }}>
                          {industry.abbrev}
                        </div>
                        <span className="text-xs font-mono" style={{ color: 'var(--muted)' }}>{String(index + 3).padStart(2, '0')}</span>
                      </div>
                      <h3 className="text-lg font-bold mb-2" style={{ color: 'var(--text)', fontFamily: 'var(--font-heading)' }}>{industry.name}</h3>
                      <p className="text-sm leading-relaxed mb-5" style={{ color: 'var(--text-secondary)' }}>{industry.description}</p>
                      <div className="flex items-center justify-between">
                        <span className="text-[0.7rem] px-2.5 py-1 rounded-full font-medium" style={{ background: 'var(--accent-light)', color: 'var(--accent)', border: '1px solid var(--border)' }}>
                          {industry.challenge}
                        </span>
                        <span className="inline-flex items-center gap-1.5 text-sm font-semibold opacity-0 group-hover:opacity-100 transition-all duration-300" style={{ color: 'var(--accent)' }}>
                          Explore
              ...
              {/* Row 3 — Small + Large featured (reversed) */}
              <div className="grid lg:grid-cols-12 gap-5">
                <Link href={industries[5].href} className="lg:col-span-4 group relative overflow-hidden rounded-2xl transition-all duration-500 hover:-translate-y-1 hover:shadow-2xl flex flex-col justify-between" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
                  <div className="absolute top-0 left-0 right-0 h-1" style={{ background: 'var(--accent)', opacity: 0.5 }} />
                  <div className="absolute -right-3 -top-2 text-[7rem] font-black leading-none select-none pointer-events-none" style={{ color: 'var(--text)', opacity: 0.03, fontFamily: 'var(--font-heading)' }}>
                    {industries[5].abbrev}
                  </div>
                  <div className="relative z-10 p-7 lg:p-8 flex flex-col h-full">
                    <span className="text-xs font-mono mb-4 block" style={{ color: 'var(--muted)' }}>06</span>
                    <h3 className="text-xl font-bold mb-3" style={{ color: 'var(--text)', fontFamily: 'var(--font-heading)' }}>{industries[5].name}</h3>
                    <p className="text-sm leading-relaxed mb-5 flex-1" style={{ color: 'var(--text-secondary)' }}>{industries[5].description}</p>
                    <div className="flex items-center justify-between mt-auto">
                      <span className="text-xs px-3 py-1.5 rounded-full font-medium" style={{ background: 'var(--accent-light)', color: 'var(--accent)', border: '1px solid var(--border)' }}>
                        {industries[5].challenge}
                      </span>
                      <svg className="w-4 h-4 transition-transform duration-300 group-hover:translate-x-1" style={{ color: 'var(--accent)' }} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 8l4 4m0 0l-4 4m4-4H3" /></svg>
                    </div>
                  </div>
                ...
                {/* CTA panel */}
                <div className="lg:col-span-8 relative overflow-hidden rounded-2xl" style={{ background: 'var(--card)', border: '1px solid var(--border)' }}>
                  <div className="absolute inset-0 pointer-events-none" style={{ backgroundImage: 'radial-gradient(circle at 1px 1px, var(--accent) 0.5px, transparent 0)', backgroundSize: '30px 30px', opacity: 0.03 }} />
                  {/* Corner brackets */}
                  <div className="absolute top-5 right-5 w-6 h-6 pointer-events-none" style={{ borderTop: '2px solid var(--accent)', borderRight: '2px solid var(--accent)', opacity: 0.2 }} />
                  <div className="absolute bottom-5 left-8 w-6 h-6 pointer-events-none" style={{ borderBottom: '2px solid var(--accent)', borderLeft: '2px solid var(--accent)', opacity: 0.2 }} />
                  ...
                  <div className="relative z-10 p-8 lg:p-12 flex flex-col justify-center h-full">
                    ...
                    <p className="text-base leading-relaxed mb-8 max-w-lg" style={{ color: 'var(--text-secondary)' }}>
                      I work with ecommerce brands across all verticals. Every strategy is custom-built for your product category, competitive landscape, and growth goals.
                    </p>
                    <div className="flex flex-wrap gap-3">
                      <Link href="/industries" className="inline-flex items-center gap-2 px-6 py-3 rounded-lg font-semibold transition-all duration-300 hover:shadow-lg" style={{ background: 'var(--accent)', color: '#ffffff' }}>
                        View All Industries
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 8l4 4m0 0l-4 4m4-4H3" /></svg>
                      </Link>
                      <Link href="/contact" className="inline-flex items-center gap-2 px-6 py-3 rounded-lg font-semibold transition-all duration-300" style={{ border: '1px solid var(--border)', color: 'var(--text)' }}>
                        Get a Free Audit
                      </Link>
                    </div>
                  </div>
